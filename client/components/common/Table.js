...
class Table extends React.Component {
   ...
   static propTypes = {
      colDef: PropTypes.arrayOf(PropTypes.shape({
         ...
         <S.Table ref={this.refTable} {...restProps}>
            ...
            <thead>
               <tr>
                  {
                     colDef.map((col, i) => (
                        <th key={i} style={col.headerStyle ? col.headerStyle : undefined}>{col.header}</th>
                     ))
                  }
               </tr>
            </thead>
                     ...
                     <tr key={i}>
                        {
                           colDef.map((col, j) => {
                              if (col.compo) {
                                 ...
                                 return (
                                    <td key={j} className={this.tdClassName(row, col)}
                                       onClick={() => col.onClick ? col.onClick({ row, col, value: row[col.field] }) : null}
