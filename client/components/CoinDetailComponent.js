...
class CoinDetailComponent extends React.Component {
   ...
   loadData = async () => {
      ...
      try {
         ...
         this.setState({
            ...
            txtCoin: '1',
            txtMoney: utils.comma3Digits(resCoin.data.market_data.current_price[currentCurrency])
         });
      }
      catch (err) {
         console.error(err);
   ...
   onChangeFilter = (kind, currency) => {
      ...
      let amount = parseFloat(txtCoin.replace(/,/g, '')) * ci.market_data.current_price[currency]
   ...
   getDescription = () => {
      ...
      if (ci.description[Language.KO] && ci.description[Language.KO] !== '') return ci.description[Language.KO]
      if (ci.description[Language.EN] && ci.description[Language.EN] !== '') return ci.description[Language.EN]
   }
   ...
   exchange = (value, toKrw) => {
      ...
      if (toKrw) {
         let krwValue = ci.market_data.current_price[currentCurrency] * parseFloat(value.replace(/,/g, ''));
         return utils.comma3Digits(parseFloat(krwValue.toFixed(2)));
      } else {
         let coinValue = parseFloat(value.replace(/,/g, '')) / ci.market_data.current_price[currentCurrency];
   ...
   render() {
      const { coinInfos: ci, bitCoinInfos: bi, currentCurrency, currentLanguage } = this.state;
      const { txtCoin, txtMoney } = this.state;
      if (!ci) return '';

      const coinPerBit = parseFloat(ci.market_data.current_price[currentCurrency] / bi.market_data.current_price[currentCurrency]).toFixed(8);
      ...
      return (
         <>
            <S.Header>
               ...
               <h3>{ci.localization[currentLanguage]} ({ci.symbol.toUpperCase()})</h3>
            ...
            <S.InfoWrap>
               <S.TableLeft>
                  <tbody>
                     ...
                     <tr>
                        <th>웹사이트</th>
                        <td><a href={ci.links.homepage[0]}>{ci.links.homepage[0]}</a></td>
                     </tr>
                  </tbody>
               </S.TableLeft>
                  ...
                  <tbody>
                     <tr>
                        <td colSpan={2} className="text-right">
                           <C.Currency currency={currentCurrency} className="current-price">
                              {utils.comma3Digits(ci.market_data.current_price[currentCurrency])}
                           </C.Currency>
                           {
                              PercentRenderer({ value: ci.market_data.price_change_percentage_24h })
                           }
                           <br />
                     ...
                     <tr>
                        <td className="text-right" style={{ paddingTop: 12 }}>
                           ...
                           <C.Currency currency={currentCurrency}>
                              {utils.comma3Digits(ci.market_data.market_cap[currentCurrency])}
                           </C.Currency>
                        </td>
                        <td className="text-right" style={{ paddingTop: 12 }}>
                           24시간 거래대금<br />
                           <C.Currency currency={currentCurrency}>
                              {utils.comma3Digits(ci.market_data.total_volume[currentCurrency])}
                           </C.Currency>
            ...
            <S.CalcBox>
               ...
               <S.Calc>
                  ...
                  <input type="text" value={txtCoin} onChange={this.onChangeTxtCoin} />
                  <img src="/images/exchange.svg" />
                  <S.Unit>{currentCurrency.toUpperCase()}</S.Unit>
                  <input type="text" value={txtMoney} onChange={this.onChangetxtMoney} />
               </S.Calc>
            </S.CalcBox>
            
            {
               ...
               <S.DescArea>
                  ...
                  <pre style={{ display: 'none' }} dangerouslySetInnerHTML={{ __html: this.getDescription() }}></pre>
               </S.DescArea>
            }
         </>
      )
